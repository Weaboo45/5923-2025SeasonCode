...
//import frc.robot.subsystems.SwerveSubsystems.Vision.Cameras;
import java.io.File;
...
public class SwerveSubsystem extends SubsystemBase
{
  ...
  public SwerveSubsystem(File directory)
  {
    ...
    try
    {
      swerveDrive = new SwerveParser(directory).createSwerveDrive(Constants.DRIVETRAIN_MAX_SPEED, startingPose);
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
  ...
  public double getRobotSpeed(){
    double speed = Math.sqrt(Math.pow(getXVelo(), 2) + Math.pow(getYVelo(), 2));
    return speed;
  }

  public double getXVelo(){
    return swerveDrive.getRobotVelocity().vxMetersPerSecond;
  }
